...
if __name__ == '__main__':
    ...
    try:
        ...
        print ('GenUefiCapsuleHeader: error: can not read binary input file {File}'.format (File = args.InputFile.name))
        sys.exit (1)
    ...
    if args.Encode:
        try:
            ...
            UefiCapsuleHeader.Payload             = Buffer
            Result = UefiCapsuleHeader.Encode ()
            if args.Verbose:
                UefiCapsuleHeader.DumpInfo ()
        except:
            print ('GenUefiCapsuleHeader: error: can not encode binary input file {File}'.format (File = args.InputFile.name))
            sys.exit (1)
    elif args.Decode:
        try:
            ...
            print ('GenUefiCapsuleHeader: error: can not decode binary input file {File}'.format (File = args.InputFile.name))
            sys.exit (1)
    elif args.DumpInfo:
        try:
        ...
        print ('GenUefiCapsuleHeader: error: invalid options')
        sys.exit (1)
    ...
    if args.OutputFile is not None:
        try:
            ...
            print ('GenUefiCapsuleHeader: error: can not write binary output file {File}'.format (File = args.OutputFile.name))
            sys.exit (1)
