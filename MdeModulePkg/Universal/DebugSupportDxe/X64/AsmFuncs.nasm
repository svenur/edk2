...
L_AppRsp: dq 0x1111111111111111 ; ?
L_DebugRsp: dq 0x2222222222222222 ; ?
L_ExtraPush: dq 0x3333333333333333 ; ?
L_ExceptData: dq 0x4444444444444444 ; ?
L_Rflags: dq 0x5555555555555555 ; ?
...
L_DebugStackEnd: db "DbgStkEnd >>>>>>"    ;; 16 byte long string - must be 16 bytes to preserve alignment
                times 0x1ffc dd    0x0  ;; 32K should be enough stack
                                        ...
                                        ;;
L_ExceptionNumber: dq 0                   ;; first entry will be the vector number pushed by the stub
...
;;
;; At this point, the stub has saved the current application stack esp into L_AppRsp
...
;; NOTE: we save rsp here to prevent compiler put rip reference cause error L_AppRsp
                push    rax
                mov     rax, qword [rsp+8]               ; save vector number
                mov     [L_ExceptionNumber], rax           ; save vector number
                pop     rax
                add     rsp, 8                           ; pop vector number
                mov     [L_AppRsp], rsp                    ; save stack top
                lea     rsp, [L_DebugStackBegin]           ; switch to debugger stack
...
;; result in a variable since we'll need this again.
                cmp     qword [L_ExceptionNumber], EXCPT64_DOUBLE_FAULT
                jz      L_ExtraPushOne
                cmp     qword [L_ExceptionNumber], EXCPT64_INVALID_TSS
                jz      L_ExtraPushOne
                cmp     qword [L_ExceptionNumber], EXCPT64_SEG_NOT_PRESENT
                jz      L_ExtraPushOne
                cmp     qword [L_ExceptionNumber], EXCPT64_STACK_FAULT
                jz      L_ExtraPushOne
                cmp     qword [L_ExceptionNumber], EXCPT64_GP_FAULT
                jz      L_ExtraPushOne
                cmp     qword [L_ExceptionNumber], EXCPT64_PAGE_FAULT
                jz      L_ExtraPushOne
                cmp     qword [L_ExceptionNumber], EXCPT64_ALIGNMENT_CHECK
                jz      L_ExtraPushOne
                mov     qword [L_ExtraPush], 0
                mov     qword [L_ExceptData], 0
                jmp     L_ExtraPushDone
L_ExtraPushOne:
                mov     qword [L_ExtraPush], 1

;; If there's some extra data, save it also, and modify the saved L_AppRsp to effectively
;; pop this value off the application's stack.
                mov     rax, [L_AppRsp]
                mov     rbx, [rax]
                mov     qword [L_ExceptData], rbx
                add     rax, 8
                mov     [L_AppRsp], rax
...
;; UINT64  RFlags;
;; L_Rflags from application is two entries back in application stack
                mov     rax, [L_AppRsp]
                push    qword [rax + 16]
...
;; UINT64  ExceptionData;
                mov     rax, [L_ExceptData]
                push    rax

; call to C code which will in turn call registered handler
; pass in the vector number
                mov     rdx, rsp
                mov     rcx, [L_ExceptionNumber]
                sub     rsp, 40
...
;; determine if we need to relocate the application stack.
                ...
                mov     rbx, [rsp + 24]  ; move the potentially modified L_AppRsp into rbx
                mov     rax, [L_AppRsp]
                mov     rax, QWORD [rax + 24]
                cmp     rbx, rax
                je      L_NoAppStackMove
                ...
                mov     rax, rbx         ; modify the saved L_AppRsp to the new L_AppRsp
                mov     [L_AppRsp], rax
L_NoAppStackMove:
                mov     rax, [L_DebugRsp]  ; restore the L_DebugRsp on the debug stack
                                         ; so our "pop" will not cause a stack switch
                mov     [rsp + 24], rax

                cmp     qword [L_ExceptionNumber], 0x68
                jne     L_NoChain
