  ...
  )
{
  ...
  TempQword =
    (MtrrRegisterRead (mMtrrLibFixedMtrrTable[MsrNum].Msr) & ~ClearMask) | OrMask;
  MtrrRegisterWrite (mMtrrLibFixedMtrrTable[MsrNum].Msr, TempQword);
  ...
  )
{
  ...
  VariableMtrrEnd = QUARK_NC_HOST_BRIDGE_IA32_MTRR_PHYSBASE0 + (2 * GetVariableMtrrCount ()) - 1;

  ZeroMem (VariableMtrr, sizeof (VARIABLE_MTRR) * MTRR_NUMBER_OF_VARIABLE_MTRR);
  UsedMtrr = 0;

  for (MsrNum = QUARK_NC_HOST_BRIDGE_IA32_MTRR_PHYSBASE0, Index = 0;
       (
    ...
    if ((MtrrRegisterRead (MsrNum + 1) & MTRR_LIB_CACHE_MTRR_ENABLED) != 0) {
      VariableMtrr[Index].Msr          = MsrNum;
      VariableMtrr[Index].BaseAddress  = (MtrrRegisterRead (MsrNum) &
                                          MtrrValidAddressMask);
      VariableMtrr[Index].Length       = ((~(MtrrRegisterRead (MsrNum + 1) &
                                             MtrrValidAddressMask)
                                          ) &
                                          MtrrValidBitsMask
                                         ) + 1;
      VariableMtrr[Index].Type         = (MtrrRegisterRead (MsrNum) & 0x0ff);
   ...
   )
{
  ...
  while (Index < VariableMtrrCount) {
    if (!VariableMtrr[Index].Valid && VariableMtrr[Index].Used) {
       MtrrRegisterWrite (VariableMtrr[Index].Msr, 0);
       MtrrRegisterWrite (VariableMtrr[Index].Msr + 1, 0);
  ...
  )
{
  ...
  MtrrRegisterWrite ((UINT32) MtrrNumber, TempQword);
  ...
  )
{
  ...
  //
  for (MsrNum = QUARK_NC_HOST_BRIDGE_IA32_MTRR_PHYSBASE0;
       MsrNum < VariableMtrrEnd;
       MsrNum += 2
      ) {
    if ((MtrrRegisterRead (MsrNum + 1) & MTRR_LIB_CACHE_MTRR_ENABLED) == 0) {
  ...
  )
{
  ...
  //
  // Get QUARK_NC_HOST_BRIDGE_IA32_MTRR_DEF_TYPE value
  //
  ...
  )
{
  ...
  //
  // Set QUARK_NC_HOST_BRIDGE_IA32_MTRR_DEF_TYPE value
  //
